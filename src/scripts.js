...
export function change_mockup(clickedImage) {
    let newSRC = clickedImage.attr("src");
    $("#item_image").attr("src", newSRC);
}
...
export function add_to_cart(node) {
...
export function remove_from_cart(node) {
    ...
    for(let key in products) {
        if(products[key].item_ID == node.attr("id")) {  //we iterate through the array until we have a matching value between the cart and the node that user requested to delete
...
//========================================================== MERCH FILE and ACCESSORY FILE FUNCTIONS =================================================================================


//this function will "slice" an element array and place the sliced array on another container element
...
export function Paginate(DIVS, container, interval) {                                 
...
export function CreatePages(DIVS, DIV_name) {
...
export function storeItem(node, description) {
    let desc = null;
    let temp = []; 
...
export function changePage(node) {
...
export function next_page() {
    ...
    if((current_page + 1) < number_of_pages.length) {                                   //this "if" statement makes sure that we do not index an element that doesnt exist in the array
        ...
        $(number_of_pages[current_page + 1]).click();                                   //we trigger the click event programatically
    }
}
...
export function sortItems() {
    ...
    if($("section select.filter_two").val() == "A-Z") {  
        ...
        $.each(sorted, (i, element) => {$("#DIV_array").append(element)} );
...
export function originalOrder (a, b) {
